	...
	int sep_len)
{
	...
	if (*matrix_split)
		*matrix_split = \
			free_matrix_line(*matrix_split, ft_matrixlen(*matrix_split) - 1);
...
char	**ft_split_strsep(char *str, char *sep, int is_char)
{
	...
	if (!str)
		return (NULL);
	init_vars_split_strsep(is_char, sep, &sep_len, &matrix_split);
	i = 0;
	while (str[i])
	{
		is_quoted = ft_is_quoted(str, i);
		if (!is_quoted && !ft_strncmp(&str[i], sep, sep_len))
			norminette_sucks(&str, &i, &matrix_split, sep_len);
